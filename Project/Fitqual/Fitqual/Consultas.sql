...
SELECT 
    LOWER(nombre_ejercicio) AS nombre_ejercicio,
    COUNT(*) AS total_rutinas,
    ROUND(AVG(series), 1) AS promedio_series,
    ROUND(AVG(repeticiones), 1) AS promedio_repeticiones
FROM Ejercicios
GROUP BY LOWER(nombre_ejercicio)
